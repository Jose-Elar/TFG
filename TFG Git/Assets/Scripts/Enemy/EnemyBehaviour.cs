...
public class EnemyBehaviour : MonoBehaviour
{
    ...
    private void Shoot()
    {
        ...
        // Spawn offset so projectile starts outside the enemy's body
        Vector3 spawnPosition = transform.position + (Vector3)(direction * spawnOffset);
 
        // Spawn as child of this enemy
        GameObject proj = Instantiate(proyectilePrefab, spawnPosition, Quaternion.identity, transform);
 
        // Rotate to face direction
        float angle = Mathf.Atan2(direction.y, direction.x) * Mathf.Rad2Deg;
        proj.transform.rotation = Quaternion.Euler(0f, 0f, angle - 90f); // Adjust if your projectile sprite faces up
 
        // Add the tracker component so it self-destructs after max distance
