...
public class MovementBehaviour : MonoBehaviour
{
    ...
    [SerializeField] private string doorWaitingDialogueId = "waiting_on_door";
    private bool _waitingAtDoor = false;
    private bool _doorDialogueFired = false;
    ...
    {
        if (!other.CompareTag("Door1")) return;

        _waitingAtDoor = true;

        if (!_doorDialogueFired)
        {
            _doorDialogueFired = true;
            if (TextManager.Instance != null)
                TextManager.Instance.StartDialogue(doorWaitingDialogueId);
        }
    }
